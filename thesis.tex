...
\begin{itemize}
\item \texttt{CONFIG\_GDB\_SCRIPTS} adds links to the GDB helper scripts.I find it particularly useful when debugging a kernel module, when I need to inspect the kernel log buffer or VFS mounts.
...
\item \texttt{CONFIG\_INIT\_DEBUG} where items marked with \_\_init (or \_\_initdata) are discarded after system initialization or module load time. This option enables checks for code that attempts to access initialization-time memory after initialization is complete.
...
\item \texttt{CONFIG\_DEBUG\_STACK\_USAGE} and \texttt{CONFIG\_DEBUG\_STACKOVERFLOW} to check the overflows of kernel, IRQ and exception stacks. This option will cause messages of the stacks in detail when free stack space drops below a certain limit.

\item \texttt{CONFIG\_KALLSYMS} causes kernel symbol information to be built into the kernel; it is enabled by default. The symbol information is used in debugging contexts; without it, an oops listing can give you a kernel traceback only in hexadecimal, which is not very useful.

\item \texttt{CONFIG\_IKCONFIG} and \texttt{CONFIG\_IKCONFIG\_PROC} (found in the ``General setup'' menu) cause the full kernel configuration state to be built into the kernel and to be made available via \/proc. Most kernel developers know which configuration they used and do not need these options (which make the kernel bigger).
\end{itemize}
