...
\begin{itemize}
\item 1427 vulnerabilities in Linux of which 427 related to the kernel;
\item 5055 vulnerabilities in Windows 10 of which 523 related to the kernel;
\item 1494 vulnerabilities in Mac Os of which 269 related to the kernel;
\end{itemize}
...
Analyzing these numbers we understand that it is increasingly necessary to analyze the kernel for vulnerabilities to be discovered before an attacker can penetrate systems and access sensitive data or to gain control of the system itself.
...
These are some of the many bugs discovered in the last five years. Consider the numbers seen above. What is scary is the constant discovery of bugs that make millions of users vulnerable, unaware of what they are using.
Therefore it is very important to have supports/tools that allow the analysis of the kernel before being placed on the market to prevent any vulnerabilities that can be exploited.
This is what we will propose in this thesis.

\section{Thesis overview}
In \refToChapter{kernel} we explained what a kernel is and how to get root privileges, in particular in \refToSection{exploitation}. Then, in order to better understand the difference between a user-land and kernel-land exploit we showed an example in \refToSection{land}.

In \refToChapter{analyze}, we described how to "set up" an environment that allows kernel analysis.
We introduced two important components that we used to carry out our task: \emph{QEMU}, in \refToSubSection{QEMU}; and \emph{GDB}, in \refToSubSection{gdb}.
In addition, we introduced the settings which we have to use in the kernel so that debugging it is as user-friendly as possible.

In \refToChapter{mitigation}, we described the kernel’s mitigations, making a distinction between those similar to user-land (see \refToSection{like user-land}) and the one created specifically for the kernel (see \refToSection{powerful mitigation}).

In order to better understand how all ingredients presented above work, in \refToChapter{adding mitigation}, we analyzed a \emph{CTF} such as an example of kernel exploitation.
We did not limit ourselves to that simple exploit, but starting from that configuration,where there are no mitigations (see \refToSection{set-up}), we developed all the possible ones as described in \refToSection{stackC}, \refToSection{addSmep}, \refToSection{trampoline}, \refToSection{addSmap} and \refToSection{kaslr-fg-kaslr}. In addition, we understood which one is able to perform the exploit and which ones not (\refToSubSection{CR4}).

\refToChapter{cve-2017} is reserved for the actual bug and its exploit present in the kernel version.
We wrote what we need in theory, \refToSection{back-cve-2017}, and in particular what happens instead in this version of the kernel in \refToSection{vulnerability_17}. We used a step by step  approach in order to better understand which ingredient is missing to have a correct functioning.
In \refToSection{bug}, we verified the bug using a customized program and then in \refToSection{exp-cve} we showed what needs to be done in the real case to penetrate the kernel.

\chapter{Kernel and its exploitation}
\label{ch:kernel}

We start our research about \emph{kernel exploitation} with this scope: explaining what the kernel is and what exploitation signifies.
...
And all this means an advantage for the \emph{attacker}. Not every bug is exploitable; we have to refer to those that are as \emph{vulnerabilities}.
...
\item The kernel is protected from user-land via both software and hardware. Finding information about the kernel is a much more difficult job. At the same time, the number of variables that are no more under the attacker’s control intensifies in an exponentially way. For example, let us consider the \emph{memory allocator}.
In a user-land exploit, the allocator is inside \emph{the process}, generally connected through a shared system library. Our purpose is its only consumer and its only \emph{affecter}. On the other side, all the processes on the system may concern the behavior and the status of a kernel memory allocator.
...
We have to start from those similar to user-land, detailed in \refToSection{like user-land} to specific ones tailored to the kernel, detailed in \refToSection{powerful mitigation}
\section{Mitigation features like user-land}
...
\emph{Finer-grained ASLR} has been proposed as a way to make \emph{ASLR} more resistantto info leaks. It is not a new concept at all, and there are many variations possible. Function reordering is an implementation of finer grained \emph{ASLR} which randomizes the layout of an address space on a function level granularity.
...
In particular, we will explore the resolution of a \emph{CTF}(catch the flag)~\cite{hxp-CTF-2020-kernel-rop}, starting from an environment without mitigations up to adding all the mitigations to solve the real \emph{CTF}.
To do this, we will use a technique called \emph{ROP}~\cite{roemer2012return} with a module having a simple bug.
...
\label{ch:cve-2017}
In this chapter we analyzed a real bug within the Linux kernel that allows us to get a root shell by performing a \emph{Local Privilege Escalation}~\cite{farah2017study}.
\section{Background}
...
\begin{lstlisting}[label={a_tasks}]
...
Note that to run this program inside GDB we used the command: source -s -v filename.py
\end{lstlisting}
...
Initially we have to create processes with the programm ~\ref{spray} which all execute the \texttt{spray_thread} function.
\begin{lstlisting}
int one_win;
...
\chapter{Conclusion}
Il crescente aumento negli ultimi anni di attacchi ai kernel dovuti a bug preoccupa il mondo dell'IT.
E' importante ridurre questa tendenza introducendo una metodologia di analisi dei kernel.
Ciò che volevamo proporre era un insieme di strumenti che permettono il debug del kernel col fine di trovare bug.
Per fare ciò abbiamo:
-spiegato cos'è un kernel e cosa significa effettuare un exploit. Abbiamo mostrato qual'è la differenza tra il mondo kernel-land e quello user-land effettuando una confronto tra due exploit.
-introdotto dei software, come QEMU e GDB, che uniti a configurazioni presenti nel kernel permettono la creazione di uno strumento d'analisi.
-parlato delle diversi mitigazioni che prevengono eventuali attacchi al kernel. Abbiamo prima introdotto quelli utilizzati anche nello user-land e poi quelli creati ad hoc per il kernel. Per verificare l'efficacia dello strumento di analisi proposto, abbiamo  effettuato diversi exploit ad un kernel con bug, attivando man mano tutte le mitigazioni ed analizzando cosa accadeva all'interno del kernel. Non sempre si può effettuare un exploit con determinate mitigazioni attive. Ciò potrebbe permettere un analisi successiva per capire come migliorare ulteriormente la protezione dei kernel proponendo possibili nuove mitigazioni. 
-utilizzato lo strumento d'analisi proposto in un caso reale di kernel affetto da bug, inizialmente per analizzare il bug e comprendere come effettuare l'exploit. Infine effettuando l'exploit del kernel.
Per ampliare e rendere più completo lo strumento proposto da questa tesi si potrebbe pensare allo sviluppo di un modus operandi/tool per la creazione di patch per kernel affetti da bug.
