...
\begin{itemize}
\item 1135 vulnerabilities in Linux related to the kernel;
\item 307 vulnerabilities in Windows related to the kernel;
\item 323 vulnerabilities in Mac Os related to the kernel;
\end{itemize}
...
\chapter{Conclusion}
In recent years there has been an increase in attacks on kernels, these are due to bugs that can endanger the whole world of IT.
It is important to reduce this trend by introducing a kernel analysis methodology.
What we wanted to propose was a set of tools that allow kernel debugging in order to find bugs.
To accomplish our goal we have:
\begin{itemize}
\item explained what a kernel is and what it means to exploit. We also showed the difference between the kernel-land and the user-land world by making a comparison between two exploits.
\item introduced software, such as QEMU and GDB, which combined with the configurations present in the kernel, allow the creation of an analysis tool.
...
To verify the effectiveness of the proposed analysis tool, we performed several exploits on a bug kernel. We tested by activating all the mitigations gradually and analyzing what happens inside the kernel. You can't always exploit with certain active mitigations. This, in fact, could allow a subsequent analysis to understand how to further improve the protection of the kernels by proposing possible new mitigations.
\item used the proposed analysis tool in a real case of a bug-affected kernel. Initially to investigate the bug and understand how to exploit it. Finally by performing the kernel exploit.
\end{itemize}

To expand and make more complete the tool proposed by us in this thesis, we could develop a modus operandi for the creation of patches for kernels affected by bugs.
