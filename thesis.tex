...
\item 307 vulnerabilities in the Windows kernel;
\item 323 vulnerabilities in the Mac Os kernel;
\end{itemize}
...
\begin{itemize}
\item The kernel is the only software strictly necessary for the system. As long as the kernel works correctly, there are no deadlocks.\vtnote{vorrei dire che finchè il kernel funziona correttamente non possono esserci problemi nel far girare il sistema operativo}
...
\chapter{An use case: mitigation inside CTF}
\label{ch:adding mitigation}
\vtnote{mi servirebbe un titolo migliore...Help me. Aggiunta intro capitolo}
In \refToChapter{analyze} we created and set up our environment. To understand if this environment allows analyzing a kernel it is necessary to test it with some use case.
In this chapter, we will test our environment with an ad hoc challenge called CTF. We will show two important aspects at the same time:
\begin{itemize}
...
\begin Observe the behaviour of our debugging environment on the different kernel settings.
\end{itemize}
...
Negli ultimi anni c'è stato un aumento degli attacchi ai kernel, dovuti a bug che possono mettere in pericolo l'intero mondo dell'IT.
Al fine di ridurre questa tendenza e al tempo stesso avere un ambiente che permetta l'analsi dei kernel prima dell'immissione sul mercato, il nostro scopo è quello di introdurre un'ambiente di debug in grado di analizzare ed adattarsi ai diversi kernel in circolazione.
Per la rete ci sono numerose guide che permettono l'exploit dei kernel affetti da bug, e pertanto il nostro scopo è anche quello di riunire sotto un unico ambiente di analisi le diverse soluzione trovate sul web.

Per raggiungere il nostro obiettivo abbiamo:
spiegato cos'è un kernel e cosa significa sfruttarlo. Abbiamo anche mostrato la differenza tra il mondo kernel-land e quello user-land facendo un confronto tra due exploit.

introdotto l'ambiente di debug andando a spiegare quali software usiamo, come QEMU e GDB, che uniti alle configurazioni presenti nel kernel, consentono la creazione di tale ambiente.

spiegato le diverse mitigazioni che impediscono possibili attacchi al kernel. Prima abbiamo introdotto quelli usati anche in user-land e poi quelli creati ad hoc per il kernel.

Per verificare l'efficacia dell'ambiente di debug proposto, siamo andati a considerare dei casi d'uso.
Il primo, utilizzando un esercitazione ad hoc, abbiamo visto come il nostro ambiente si adatti bene alle diverse configurazioni del kernel e come permette un analisi efficace rendendo tutto molto semplice ed intuitivo. Nello specifico abbiamo anche osservato come non sempre è possibile effettuare un exploit avendo attive diverse mitigazioni.
Il secondo, andando a considerare un reale bug presente in un kernel, per verificare se il nostro ambiente funziona in casi reali oppure no.
La scelta di comporre l'ambiente di debug con i software utilizzata è fatta sulla base che sono facili da istallare e semplici da utilizzare e permettono di ridurre le risorse utilizzate per la fase di debug avendo delle ottime prestazioni.

Possiamo concludere che il nostro ambiente di debug si comporta bene con diversi kernel e in diverse situazioni. La sua adattabilità lo rende un buono strumento per le diverse esigenze degli utenti e seaffiancato da un ambiente che permetta la creazione di patch per kernel affetti da bug potrebbe essere una suite completa per l'analisi dei kernel prima dell'immissione sul mercato.
