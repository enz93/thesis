...
\begin{itemize}
\item \texttt{-kernel  "path"}, (where path means the) Path to kernel image to debug;
\item \texttt{-initrd "path"}, Path to initial Ram disk. In short, a RAM disk is a filesystem dynamically placed in you RAM memory in boot time that contains the basic stuff needed to get your real filesystem running with the first processes needed to get your whole system running as expected, like the init process;
\item \texttt{-gdb dev} , wait for gdb connection on device dev. Typical connections will likely be TCP-based, but also UDP, pseudo TTY, or even stdio are reasonable use case;
...
\item \texttt{-cpu model}, select CPU model. Here we can add +smep and +smap for \emph{SMEP} and \emph{SMAP} mitigation features;
\item \texttt{-m [size=]megs}, set virtual RAM size to megs megabytes;
