...
In user space we had the support of the kernel so we could easily stop processes and use gdb to inspect their behavior.\glnote{perché al passato? In user space ce l'abbiamo ancora ;)}
...
In the kernel, in order to use gdb we need to use hypervisor like QEMU based hardware interfaces which are not always available.\glnote{non capisco la frase}
...
\label{subsect:QEMU}
As said previously, to debug the kernel we use the qemu\glnote{il nome è QEMU, tutto maiuscolo (cambia ovunque)} hypervisor. Specifically, there are some options needed in kernel analysis:
\begin{itemize}
...
\item \texttt{-gdb }\textit{dev}\glnote{per le ``variabili'' meglio textit}, wait for gdb connection on device \textit{dev}. Typical connections will likely be TCP-based, but also UDP, pseudo TTY, or even stdio are reasonable use case;
...
\item \texttt{-cpu model}, select CPU model. Here we can add \texttt{+smep} and \texttt{+smap} for \emph{SMEP} and \emph{SMAP} mitigation features;\glnote{riferimento alla/e sezione/i dove ne parli}
...
\glnote{mi sembra più sensato come sezione del capitolo precedente che come capitolo a se stante}
We recommended that you build and install your own kernel, rather than running the stock kernel that comes with your distribution. One of the strongest reasons for running your own kernel is that the kernel developers have built several debugging features into the kernel itself. These features can create extra output and slow performance, so they tend not to be enabled in production kernels from distributors.\glnote{questo pezzo è copia-incollato!}

When building a kernel for debugging with gdb, I would advise using the following configuration options to make debugging a bit more pleasant. \glnote{non usare il dobbio backslash per andare a capo} \\
Except where specified otherwise, all of these options are found under the \emph{``kernel hacking"} menu in whatever kernel configuration tool you prefer. Note that some of these options are not supported by all architectures and even if they are added, they may be not considered for the building.\glnote{ho cambiato un po' l'inglese, era questo il senso?}
\begin{itemize}
\item \texttt{CONFIG\_GDB\_SCRIPTS} adds links to the GDB helper scripts. I\glnote{un po' I un po' we\ldots uniformare; in genere si usa ``we''; per una tesi ok anche I, ma che sia consistente} find it particularly useful when debugging a kernel module, when I need to inspect the kernel log buffer or VFS mounts.
...
\end{itemize}\glnote{anche questa lista di opzioni ``puzza'' di copia-incolla, ne parliamo a voce}
...
\end{lstlisting} qemu-system-i386 -hda archlinux-i686.img -boot d -cdrom ../archlinux
.12.01-dual.iso -m 1024
\chapter{Kernel exploitation - CVE-2017}
\chapter{Conclusion}
